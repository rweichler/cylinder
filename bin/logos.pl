...
foreach my $line (@lines) {
	...
	while($line =~ m/\B(?=(\%\w|&\s*\%\w|[+-]\s*\(\s*.*?\s*\)))/gc) {
		...
		if($line =~ /\G%hook\s+([\$_\w]+)/gc) {
			...
			nestingMustNotContain($lineno, "%hook", \@nestingstack, "hook", "subclass", "hookC");
			...
			patchHere(undef);
		} elsif($line =~ /\G%hookC\b/gc) {
			#im high on drugs!!!!
			fileError($lineno, "%hookC does not make sense inside a block") if($directiveDepth >= 1);
			nestingMustNotContain($lineno, "%hookC", \@nestingstack, "hook", "subclass", "hook");

			@firstDirectivePosition = ($lineno, $-[0]) if !@firstDirectivePosition;

			nestPush("hookC", $lineno, \@nestingstack);
			...
			nestingMustNotContain($lineno, "%subclass", \@nestingstack, "hook", "subclass", "hookC");
